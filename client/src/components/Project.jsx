...
import { IoRocketSharp, IoLogoReact, IoLogoNodejs, IoLogoHtml5, IoLogoCss3 } from "react-icons/io5";
...
const projects = [
  {
    ...
    github: '',
    tech: [<SiMongodb />, <SiExpress />, <IoLogoReact />, <IoLogoNodejs />, <SiTailwindcss />]
  },
  {
    name: 'Dalle Clone',
    img: dalle,
...
const Project = () => {
  return (
    <section id='project' className='bg-black text-white min-h-screen h-auto py-10'>
            ...
            <div className='front flex-1 flex flex-col gap-y-12 mb-10 lg:mb-0'>
              ...
              <div className='flex justify-end'>
                <div>
                  ...
                  <motion.p 
                    ...
                    Step into a realm of creativity and inspiration with my recent projects.
                  </motion.p>
                  {/* button */}
                  {/* <button className=''>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {projects.map((project, index) => (
                  <div className="w-full md:w-[300px] lg:w-[400px] h-[250px] group relative overflow-hidden border-2 border-white/50 rounded-xl cursor-pointer" key={index}>
                    {/* overlay */}
                    <div className="group-hover:bg-black/70 w-full h-full absolute z-40 transition-all duration-300"></div>
                    {/* img */}
                    <img className="w-full h-full group-hover:scale-150 transition-all duration-500" src={project.img} alt={project.img} />
                    {/* github live */}
                    <div className="flex absolute -bottom-full left-56 lg:left-80 gap-2 group-hover:bottom-52 transition-all duration-500 z-50">
                      <a href={project.github}>
                        <div className="text-xl"><GoMarkGithub /></div>
                      </a>
                      <a href={project.src}>
                        <div className="text-xl"><IoRocketSharp /></div>
                      </a>
                    </div>
                    {/* pretitle */}
                    ...
                    <div className="absolute -bottom-full left-16 lg:left-28 group-hover:bottom-20 transition-all duration-700 z-50">
                      <span className="text-gradient text-4xl">{project.name}</span>
                    </div>
                    {/* texh stack */}
